import { update } from '@/actions/App/Http/Controllers/DrTestController';
...
export default function Edit({ drTest }: EditProps) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        },
        {
            ...
            href: show(drTest.id).url,
        },
        {
            title: 'Edit',
            href: `/dr-tests/${drTest.id}/edit`,
        },
    ];
    ...
    const { data, setData, put, processing, errors } = useForm<{
        ...
        test_date: drTest.test_date,
        rto_minutes: String(drTest.rto_minutes),
        rpo_minutes: String(drTest.rpo_minutes),
        notes: drTest.notes ?? '',
        phases: drTest.phases.map((phase) => ({
            id: phase.id,
            title: phase.title,
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        put(update(drTest.id).url);
    }

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit DR Test - ${drTest.test_date}`} />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild>
                        <Link href={show(drTest.id).url}>
                ...
                <h1 className="text-2xl font-bold">Edit DR Test</h1>
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex items-center gap-4">
                        <Button type="submit" disabled={processing}>
                            {processing ? 'Saving...' : 'Update DR Test'}
                        </Button>
                    </div>
                </form>
            </div>
